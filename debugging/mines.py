...
class Minesweeper:
    def __init__(self, width=10, height=10, mines=10):
        ...
        self.safe_cells = self.total_cells - mines  # cells that are not mines
    ...
    def reveal(self, x, y):
        # Prevent re-revealing the same cell
        if self.revealed[y][x]:
            return True

        # If clicked a mine → lose
        if (y * self.width + x) in self.mines:
            return False

        # Mark as revealed safe cell
        self.revealed[y][x] = True
        self.revealed_count += 1

        # Auto-reveal neighbors if zero mines around
